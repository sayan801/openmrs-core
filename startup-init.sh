...
# --- CUSTOM OMOD COPY BLOCK ---
CUSTOM_OMODS_DIR="${OMRS_HOME}/custom_omods"
if [ -d "$CUSTOM_OMODS_DIR" ]; then
  echo "Checking for custom omods in $CUSTOM_OMODS_DIR..."
  if [ -z "$(ls -A "$OMRS_MODULES_DIR")" ]; then
    echo "Module directory is empty, copying custom omods..."
    cp -v "$CUSTOM_OMODS_DIR"/*.omod "$OMRS_MODULES_DIR/" || echo "No .omod files found in custom_omods."
  else
    echo "Module directory not empty, skipping custom omod copy."
  fi
fi
# --- END CUSTOM BLOCK ---
